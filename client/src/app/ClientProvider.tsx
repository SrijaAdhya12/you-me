...
export default function ClientProvider({ children }: ClientProviderProps) {
    ...
    if (!videoClient) return <div className="h-screen flex items-center justify-center">
...
const useInitializeVideoClient = () => {
    ...
    useEffect(() => {
        ...
        let streamUser: User
        if (user?.id) { streamUser = { id: user.id, name: user.username || user.id, image: user.imageUrl }; }
        else {
            const id = nanoid();
            streamUser = {
                id,
